...
private:
	...
	void clearRec(TreeNode*);
	void copyRec(TreeNode*);
...
void BinaryTree<T>::deleteNode(T item, TreeNode *&nodePtr) {
	if (item < nodePtr->value) {
		if (nodePtr->left)
			...
			cout << "Value " << item << " not found in tree. No nodes were deleted." << endl;
	}
	else if (item > nodePtr->value) {
		if (nodePtr->right)
...
//****************************************************************
// This function recursively places the contents of a binary tree
// into an array with inorder traversal.
//****************************************************************
template<class T>
...
//****************************************************************
// This function checks if the tree making the function call is
// empty and either calls the above function with the root as the
// first parameter or throws an error string.
//****************************************************************
template<class T>
void BinaryTree<T>::loadInOrder(T A[]) {
	if (root) {
		int start = 0;
		loadInOrderRec(root, A, start);
	}
	else {
		string error = "Error: Tree is Empty.";
		throw error;
	}
}
...
int BinaryTree<T>::numberOfNodes() {
	if (root)
		return 1 + numberOfNodesRec(root);
	else
		return 0;
}
...
int BinaryTree<T>::numberOfNodesRec(TreeNode* nodePtr) {
	...
	if (nodePtr->left)
		count += (1 + numberOfNodesRec(nodePtr->left));
	if (nodePtr->right)
		count += (1 + numberOfNodesRec(nodePtr->right));
...
int BinaryTree<T>::numberOfLeafNodesRec(TreeNode* nodePtr) {
	if (!nodePtr->left && !nodePtr->right)
		...
		int count = 0;
		if (nodePtr->left)
			count += numberOfLeafNodesRec(nodePtr->left);
		if (nodePtr->right)
			count += numberOfLeafNodesRec(nodePtr->right);
		return count;
	}
}
...
int BinaryTree<T>::heightRec(TreeNode* nodePtr) {
	...
	if (nodePtr->left)
		lSubHeight = heightRec(nodePtr->left);
	if (nodePtr->right)
		rSubHeight = heightRec(nodePtr->right);
...
template<class T>
void BinaryTree<T>::clearRec(TreeNode* nodePtr) {
	if (nodePtr) {
		if (nodePtr->left) {
			clearRec(nodePtr->left);
		}
		if (nodePtr->right) {
			clearRec(nodePtr->right);
		}
		delete nodePtr;
	}
}
...
template<class T>
BinaryTree<T>& BinaryTree<T>::operator=(BinaryTree& rhs) {
	if (root)
		clear();
	if (rhs.root)
		copyRec(rhs.root);
...
template<class T>
BinaryTree<T>::BinaryTree(BinaryTree& copy) {
	if (copy.root)
		copyRec(copy.root);
}

template<class T>
void BinaryTree<T>::copyRec(TreeNode* nodePtr) {
	insertNode(nodePtr->value);
	if (nodePtr->left)
		copyRec(nodePtr->left);
	if (nodePtr->right)
		copyRec(nodePtr->right);
}
