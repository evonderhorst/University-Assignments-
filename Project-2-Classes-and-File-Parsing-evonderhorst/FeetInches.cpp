...
void FeetInches::setLength(string length) {
	int divider = length.find("'");
	try {
		feet = stoi(length.substr(0, divider));
	} catch (exception& e) {
		feet = 0;
	}

	length.erase(0, divider + 1);
	divider = length.find('"');
	try {
		inches = stoi(length.substr(0, divider));
	} catch (exception& e) {
		inches = 0;
	}
...
string FeetInches::toString() const {
